...
// The Stack, which seems like a bind operator
// M a -> (a -> M b) -> M b

struct StackLayer<Outer, Inner> {
    outer: Outer,
    inner: Inner,
}

impl<S, Outer, Inner> Layer<S> for StackLayer<Outer, Inner>
    where
        Inner: Layer<S>,
        Outer: Layer<Inner::Service>,
{
    ...
    fn layer(&self, s: S) -> Self::Service {
        let inner_s = self.inner.layer(s);
        self.outer.layer(inner_s)
    }
}
...
async fn main() {
    ...
    let mock_server = async move {
        let log_and_delay = StackLayer {
            inner: TimeoutLayer{ duration: time::Duration::new(1, 0) },
            outer: LogLayer{ log_str: "Echo" },
        };
        let mut log_delayed_echo = log_and_delay.layer(EchoService{});
        for _ in 0..iter_num {
