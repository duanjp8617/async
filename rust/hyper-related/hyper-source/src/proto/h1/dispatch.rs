...
{
    pub fn new(dispatch: D, conn: Conn<I, Bs::Data, T>, id:i32) -> Self {
        Dispatcher {
            conn,
            dispatch,
            body_tx: None,
    ...
    fn poll_loop(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        info!("Dispatch {}: running poll loop", self.id);
    ...
    Bs::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        // info!("Dispatcher {}: polling", self.id);
...
impl<B> Client<B> {
    pub fn new(rx: ClientRx<B>, id: i32) -> Client<B> {
        Client {
            callback: None,
            rx,
            rx_closed: false,
