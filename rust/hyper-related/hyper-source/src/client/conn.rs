...
where
    B: HttpBody,
{
    H1(#[pin] Http1Dispatcher<T, B, proto::h1::ClientTransaction>, i32),
...
{
    Builder::new().handshake(io, 0).await
}
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    pub fn into_parts(self) -> Parts<T> {
        let (io, read_buf, _) = match self.inner.expect("already upgraded") {
            ProtoClient::H1(h1, _) => h1.into_inner(),
            ProtoClient::H2(_h2) => {
    ...
    pub fn poll_without_shutdown(&mut self, cx: &mut task::Context<'_>) -> Poll<crate::Result<()>> {
        match *self.inner.as_mut().expect("already upgraded") {
            ProtoClient::H1(ref mut h1, _) => h1.poll_without_shutdown(cx),
        ...
        B::Error: Into<Box<dyn StdError + Send + Sync>>,
    {
        ...
        async move {
            ...
            let proto = if !opts.http2 {
                let mut conn = proto::Conn::new(io, id);
                ...
                let cd = proto::h1::dispatch::Client::new(rx, id);
                let dispatch = proto::h1::Dispatcher::new(cd, conn, id);
                ProtoClient::H1(dispatch, id)
            } else {
            ...
            Ok((
                SendRequest { dispatch: tx, id },
                Connection { inner: Some(proto), id },
            ))
        }
    }
}
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        match self.project() {
            ProtoClientProj::H1(c, _) => c.poll(cx),
