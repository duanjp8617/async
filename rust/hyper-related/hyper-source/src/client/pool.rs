...
impl<T: Poolable> Pool<T> {
    ...
    pub fn checkout(&self, key: Key, id: i32) -> Checkout<T> {
        Checkout {
            key,
            pool: self.clone(),
            waiter: None,
    ...
    pub(super) fn connecting(&self, key: &Key, ver: Ver, id: i32) -> Option<Connecting<T>> {
        if ver == Ver::Http2 {
        ...
        info!("ReqID {}: it seems that this request is using HTTP1", id);
        // else
        Some(Connecting {
            key: key.clone(),
...
impl<T: Poolable> Checkout<T> {
    fn poll_waiter(
        ...
        if let Some(mut rx) = self.waiter.take() {
            ...
            info!("ReqID {}: Checkout has no waiter", self.id);
            Poll::Ready(None)
        }
    }
    ...
    fn checkout(&mut self, cx: &mut task::Context<'_>) -> Option<Pooled<T>> {
        let entry = {
            info!("ReqID {}: checkout with key {:?}", self.id, self.key);
            ...
            let (entry, empty) = if let Some((e, empty)) = maybe_entry {
                ...
                info!("ReqID {}: found no existing conneciton", self.id);
            ...
            if entry.is_none() && self.waiter.is_none() {
                ...
                info!("ReqID {}: pushing a notifying into the VecDeque", self.id);
                inner_vd.push_back(tx);
...
impl<T: Poolable> Future for Checkout<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        ...
        if let Some(pooled) = self.checkout(cx) {
            ...
            info!("ReqID {}: the first pool of Checkout returns pending", self.id);
...
impl<T: Poolable> Connecting<T> {
    pub(super) fn alpn_h2(self, pool: &Pool<T>) -> Option<Self> {
        ...
        pool.connecting(&self.key, Ver::Http2, 0)
    }
}
