    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    /// # fn main() {}
    /// ```
    pub fn get(&self, uri: Uri, id: i32) -> ResponseFuture
    where
        B: Default,
    {
        let body = B::default();
    ...
    /// # fn main() {}
    /// ```
    pub fn request(&self, mut req: Request<B>, id: i32) -> ResponseFuture {
        ...
        ResponseFuture::new(Box::new(self.retryably_send_request(req, pool_key, id)))
    }
    ...
    fn retryably_send_request(
        ...
        let mut send_fut = client.send_request(req, pool_key.clone(), id);
            ...
            match ready!(Pin::new(&mut send_fut).poll(cx)) {
                ...
                Err(ClientError::Canceled {
                    ...
                    *req.uri_mut() = uri.clone();
                    send_fut = client.send_request(req, pool_key.clone(), id);
                }
            }
        })
    }
    ...
    fn send_request(
        ...
        let conn = self.connection_for(pool_key, id);
    ...
    fn connection_for(
        ...
        let checkout = self.pool.checkout(pool_key.clone(), id);
        let connect = self.connect_to(pool_key, id);
    ...
    fn connect_to(
        ...
        hyper_lazy(move || {
            ...
            let connecting = match pool.connecting(&pool_key, ver, id) {
                Some(lock) => lock,
                None => {
                    let canceled =
            ...
            Either::Left(
                connector
                    ...
                    .and_then(move |io| {
                        info!("ReqID {}: get the connected TcpStream", id);
